...
export function AlarmCard({ alarm, onToggle, onEdit, selectMode, selected, onSelect }: Props) {
  const type = getType(alarm.typeId);
  const snd  = getSound(alarm.snd ?? 'default');
  const vib  = getVib(alarm.vib);
  const showDate = alarm.sd && alarm.sd !== todayStr();

  return (
    <TouchableOpacity
      style={[s.card, alarm.active ? s.cardOn : s.cardOff]}
    ...
    >
      ...
      <View style={s.info}>
        ...
        {showDate && (
          <Text style={s.dateText}>📅 {fmtDisplayDate(alarm.sd)}부터</Text>
        )}
        <View style={s.badges}>
...
const s = StyleSheet.create({
  card: {
    ...
    shadowOpacity:0.07, shadowRadius:6, elevation:2, borderWidth:1,
  },
